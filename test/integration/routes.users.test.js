...
describe('routes : users', () => {
  ...
  describe('POST /api/v1/users/', () => {
    it('should respond with a success message along with the single user that was added', (done) => {
      chai.request(server)
      .post('/api/v1/users')
      .send({
        username: 'Jimmy',
        email: 'jimmyh@jimmy.com'
      })
      .end((err, res) => {
        // there should be no errors
        should.not.exist(err);
        // there should be a 201 status code
        //indicating something was created
  ...
  describe('DELETE /api/v1/users/:id', () => {
    it('should respond with a success message along with the single user that was deleted', (done) => {
      knex('users')
      .select('*')
      .then((user) => {
        const userObject = user[0];
        const lengthBeforeDelete = user.length;
        chai.request(server)
